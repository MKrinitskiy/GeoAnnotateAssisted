...
class CSVDataset(Dataset):
    ...
    def __init__(self, train_file, class_list, filename, transform=None):
        """
        Args:
...
class CSVDataset_threaded(Dataset):
    def __init__(self, train_file, class_list, filename, batch_size=1, shuffle=True, transform=None):
        """
        Args:
            train_file (string): CSV file with training annotations
            annotations (string): CSV file with class list
    ...
    def __len__(self):
        l = len(self.image_names)/self.batch_size
        if ((l - int(l))>0.0):
            l = int(l)+1
        return int(l)
    ...
    def __iter__(self):
        while True:
            ...
            for idx in self.objects_id_generator:
                ...
                annot = self.load_annotations(self.obj_indices[idx])
                sample = {'img': img, 'annot': annot}
                if self.transform:
                    sample = self.transform(sample)
