...
class SourceDataPlotter_METEOSAT_MCS(SourceDataPlotter):
    def __init__(self, parent):
...
class SourceDataPlotter_NAAD_PL(SourceDataPlotter):
    def __init__(self, parent):
        super().__init__(parent)
    ...
    def PlotBasemapBackground(self):
        ...
        cs = self.parent.bm.contour(self.parent.sourceDataManager.lons, self.parent.sourceDataManager.lats,
                                    self.parent.sourceDataManager.data['msl'], latlon=True, levels=np.arange(norm_constants.msl_vmin, norm_constants.msl_vmax, 5.0),
